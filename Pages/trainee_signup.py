    ...
    with ui.card().classes('w-full max-w-md mx-auto my-8 rounded-xl shadow-2xl'):
        with ui.column().classes('w-full p-8 gap-4'):
            ...
            with ui.row().classes('w-full grid grid-cols-2 gap-4 mb-4'):
                with ui.column().on('click', lambda: ui.navigate.to('/agent/signup')).classes('group items-center justify-center p-4 border-2 rounded-lg cursor-pointer border-gray-300 hover:border-indigo-500 text-gray-500 hover:text-gray-700 transition-all'):
                    ui.icon('support_agent').classes('text-4xl mb-2 text-gray-400 group-hover:text-indigo-500')
            ...
            ui.button('Sign Up', on_click=lambda: ui.notify('Creating Trainee Account...')).classes('w-full bg-indigo-500 text-white font-bold py-3 px-4 rounded-lg hover:bg-indigo-600 transition-colors')
